...
const OPTION_LABELS = ['A', 'B', 'C', 'D'];
  ...
  // Helper: get teacher name from localStorage profile
  function getTeacherNameFromProfile(email) {
    try {
  ...
  const [uploadTeacher, setUploadTeacher] = useState(
    getTeacherNameFromProfile(teacherSession?.email) || teacherSession?.name || teacherSession?.email || ''
  );
  ...
  // Sync uploadTeacher with current session: prefer profile name > session name > email
  useEffect(() => {
    if (teacherSession?.email) {
      const profileName = getTeacherNameFromProfile(teacherSession.email);
      setUploadTeacher(profileName || teacherSession.name || teacherSession.email);
    }
  }, [teacherSession?.email, teacherSession?.name]);
                ...
                <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 rounded-xl px-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-semibold text-slate-800 block truncate">
                      {uploadTeacher || 'Chưa có họ tên — cập nhật Hồ sơ GV'}
                    </span>
                    {teacherSession?.email && (
                      <span className="text-[10px] text-slate-400 block truncate">{teacherSession.email}</span>
                    )}
                  </div>
                  <span className="text-[10px] text-slate-400 font-medium flex-shrink-0">(tự động)</span>
                </div>
                <p className="text-[10px] text-slate-400 mt-1">Cập nhật họ tên tại <strong className="text-amber-600">Hồ sơ GV</strong> để thay đổi.</p>
              </div>
